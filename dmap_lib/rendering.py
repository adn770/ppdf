...
def _generate_hatching(pixel_contour: np.ndarray, density: float) -> list[str]:
    ...
    hatch_lines = []
    # Ensure contour is in the shape (N, 2)
    ...
    for i in range(len(edges) - 1):
        ...
        for _ in range(num_hatches):
            ...
            mid_x, mid_y = p1[0] + dx * r, p1[1] + dy * r
...
def render_svg(map_data: schema.MapData, unified_contours: list | None, style_options: dict) -> str:
    ...
    if unified_contours:
        for contour in unified_contours:
            hatching.extend(_generate_hatching(contour, styles["hatch_density"]))

    # Render rooms (without hatching)
    shadows, glows, walls = [], [], []
    for obj in objects:
        if isinstance(obj, schema.Room):
            ...
            walls.append(f'<polygon points="{points}" fill="{styles["room_color"]}" stroke="{styles["wall_color"]}" stroke-width="{lt}"/>')
    ...
    svg.append(f'<g id="walls">{"".join(walls)}</g>')
    svg.extend(['</g>', '</svg>'])
    return "\n".join(svg)
